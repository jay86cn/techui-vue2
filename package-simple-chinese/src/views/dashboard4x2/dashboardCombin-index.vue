    ...
    if(this.deviceType!="mobile"){
      ...
      if(tipState){
        $timer("notify1",()=>{
          $notify({
            title: '性能提示',
            message: '数据组合屏专为企业级的大屏展示而打造，需要更高性能的环境运行，因同时展现的图表数据过多，面对一些弱性能的个人电脑环境较为吃力，如遇到卡顿，请更换电脑再试。也可尝试在控制面板中关闭背景模糊、动画效果等，提高页面效率。控制面板开关位于窗口右上角',
            duration: 0
          });
        },3000)
        $timer("notify2",()=>{
          $notify({
            title: '提示',
            message: '可以通过拖拽右下角的面板，调换4x2拼接屏中的中间主屏内容。可以根据情况定制任意分辨率的大屏。',
            duration: 0
          });
        },5000)
