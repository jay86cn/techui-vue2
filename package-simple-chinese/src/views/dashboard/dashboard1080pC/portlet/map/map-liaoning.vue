  ...
  data() {
    return {
      ...
      chartData:{
        ...
        sum:0,
        nav:{
          data:[{code:210000,name:"辽宁"}, ],//{code:100000,name:"全国"},{code:210000,name:"辽宁省"},
          position:{
            bottom:"28%",
            left:"27.5%"
          },
    ...
    mapClick(params){
      ...
      if(navDept[navDept.length-1].name!=params.name){
        if(params.componentType=="geo"||(params.componentType=="series"&&params.componentSubType=='map')){//scatter
          ...
          if(navDept[navDept.length-1].name!=params.name&&navDept.length<2){//&&navDept.length<2
            this.initChart(obj)
