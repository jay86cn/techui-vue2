...
export default {
  ...
  methods:{
    getPalette(op){
      if(!op.palette){
        op.palette=this.$vuex.state.palette.chart.palette_A
      }else if(op.palette=="alt"){
        op.palette=this.$vuex.state.palette.chart.palette_B
      }
      return A("cp",op)
    },
    viewColor(arr){
    ...
    getColor(val){//改造charttheme，生成一个全局的chart 配色工具
      const {palette}=this.$vuex.state
      let {chart,font,tech}=palette,
          cl=this.mapColorOri;
      switch(val){
        case "mapBGA":
