<template>
  <div class="screen1080AG" :style="svgBG1" >
      ...
      <board3D :config="getBorder3DConfig(area.name)" class="board-3d">
        <div class="portlet-wrapper" v-for="(item,index) in area.portlets">
          <component v-if="item.border" :is='item.border' :config="getConfig(item)">
            <blockTitleA1 v-if="!item.hideTitle" :config="blockTitleConfig" >{{$t(`portlet.title.${item.id}`)}}</blockTitleA1>
  ...
  methods: {
    ...
    getConfig(item){
      ...
      if(id=='r3'){
        ...
        return {
          title:this.$t(`portlet.title.${item.id}`),
          titleWidth:120,
          decoration:false,
          decorationAlt: true,
          rotate: "y",
...
<style lang="less">
.screen1080AG{ z-index: 1; padding:70px 60px 50px 60px;
