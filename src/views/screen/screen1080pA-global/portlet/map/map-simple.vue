...
export default {
  ...
  components: {
    ...
    mapCounter:() => import('./map-counter/map-counter'),
    echartPieMini:() => import('./echarts-PieMini'),
  ...
  methods: {
    ...
    getMapData(current) {//2nd 生成地图假数据 ------------ 开发过程中，这个方法要替换成根据地图参数获取服务器数据的方法！
      ...
      chartData.geoJson.features.forEach((geo,i) => {
        if(geo.properties.name){
          let value = A("rn",1,chartData.maxData/3)
          let maxValue = A("rn",chartData.maxData/2,chartData.maxData)
          // mapData.push({
    ...
    async initChart(current) {//1st 从地图文件 获取geojson数据 
      ...
      await map(this.publicUrl,current.code+'_geojson_full.json').then((res) => {
        this.$vuex.state.mapData.code=current.code;
        this.$vuex.state.mapData.name=current.name;
