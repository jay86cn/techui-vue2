<template>
  <div class="screenAG-counterGrid">
...
export default {
  ...
  mounted() {
    this.arry.forEach((item,i)=>{
      item.title=this.$t("portlet.r1.counter"+i);
      item.unit=this.$t("portlet.r1.unit");
    })
  }
};
</script>
<style lang="less">
.screenAG-counterGrid{display: grid; grid-template-columns: repeat(2,1fr); height: 100%; grid-template-rows: repeat(2,1fr); grid-gap: 12px;  position: relative;
