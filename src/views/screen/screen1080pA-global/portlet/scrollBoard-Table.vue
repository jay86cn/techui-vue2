<template>
    <dv-scroll-board v-if="!option.reset" :config="option" class="screenAG-scrollBoard-table"  />
</template>
<script>
export default {
  name: "blockCenter",
  ...
  watch:{
    "$vuex.state.mapData.code"(val1,val2){
      ...
      A("tis","resetDataTAble",()=>{this.option.reset=false;},100)
    }
  },
  methods: {
    processData(){
...
<style lang="less">
.screenAG-scrollBoard-table{.poa; left:10px; right:10px; top:40px; bottom:10px; width:auto;height:auto; 
  .header{
    .header-item{.ff("cn1"); }
  }
  .rows{
