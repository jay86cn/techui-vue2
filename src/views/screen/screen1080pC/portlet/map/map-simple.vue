...
export default {
  ...
  data() {
    return {
      ...
      chartData:{
        alladcode:[],
        geoJson:{},
        dept:[{code:100000,name:"全国"}, ],//{code:100000,name:"全国"},{code:210000,name:"辽宁省"},
  ...
  methods: {
    ...
    processMapData(current){//4th 处理时间线的 柱状图数据
      ...
      const {mapData,pointData,sum,dept,maxData,minData,maxPin,minPin}=chartData
  ...
  mounted() {
    ...
    // cl("process.env.NODE_ENV",process.env.VUE_APP_MAP_BASE);
