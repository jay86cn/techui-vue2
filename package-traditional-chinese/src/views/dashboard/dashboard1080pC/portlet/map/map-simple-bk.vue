    ...
    processOption(current){//3rd 初步处理图表配置项
      ...
      let tempOption={
        ...
        geo:{
          ...
          //zoom :1.2,
          layoutCenter:chartData.navDept.length === 1 ?  ['50%', '65%'] :  ['50%', '50%'],
          layoutSize:chartData.navDept.length === 1 ? "95%" : "85%",
          zlevel:1,
          label: {
            show: true,
      ...
      chartData.geoJson.features.forEach(j => {
        if(j.properties.name){
          ...
          if(j.properties.center){
            let dataArea=["北京市","遼寧省","内蒙古自治区","四川省","广东省"],//伪造几个特定的top5区域
...
<style  lang="less">
.dashboard1080pC-map{.poa; .fullbox;
