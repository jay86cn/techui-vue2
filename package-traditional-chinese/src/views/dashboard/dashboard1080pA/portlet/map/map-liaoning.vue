  ...
  data() {
    return {
      ...
      chartData:{
        ...
        nav:{
          data:[{code:210000,name:"遼寧"}, ], //{code:100000,name:"全国"},  {code:140000,name:"山西省"},
          position:{
            top:"18%",
            left:"27.5%"
          },
        ...
        if(geo.properties.name){
          ...
          if(geo.properties.center){
            ...
            if(warnIndex.includes(i)){
              warningData.push({
                name: geo.properties.name,
                value: [geo.properties.center[0], geo.properties.center[1]-.3, 'warning',"區域數據觸發異常告警"],
                cityCode: parseInt(geo.properties.adcode)
              })
            }
            if(dangerIndex.includes(i)){
              dangerData.push({
                name: geo.properties.name,
                value: [geo.properties.center[0], geo.properties.center[1]-.1, 'danger',"區域數據觸發危急告警"],
                cityCode: parseInt(geo.properties.adcode)
              })
            }
          }
