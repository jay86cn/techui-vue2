...
export default {
  ...
  mixins: [themeChart],
  components: {
    mapPopover:() => import("../map/map-popover.vue"),
    mapCounter:() => import('../map/map-counter/map-counter.vue'),
    echartPieMini:() => import('../map/echarts-PieMini.vue'),
    mapViewToggle:() => import('../map/map-viewToggle.vue'),
  },
  data() {
    return {
      mapInited:false,
      ...
      chartData:{
        ...
        layoutSize:"95%",
        nav:{
          data:[{code:100000,name:"全国"},],//{code:100000,name:"全国"},  {code:140000,name:"山西省"},
          position:{
            top:"18%",
            left:"27.5%"
          },
  ...
  methods: {
      ...
      mapD.push(
        ...
        },
        {
          ...
          label: {//ayinMark
            ...
            formatter:p=>{
              return p.data?.replace(/省|壮族自治区|回族自治区|维吾尔自治区|自治区|市|县|自治县|特别行政区|区/, '')
            },
            position: 'center',
            align:"center",
            show: false,
    ...
    processOption(current){//3rd 初步处理图表配置项
      ...
      const {chartData,chartOption,processMapData,getColor}=this,{visualMap}=chartData
      let tempOption={
        title: {
          show:false,
          text: current.name,
      ...
      tempOption.visualMap={
        ...
        right:visualMap.position.right,
        bottom:visualMap.position.bottom,
        min:0,
        ...
        if(geo.properties.name){
          ...
          if(geo.properties.center){
            ...
            if(warnIndex.includes(i)){
              warningData.push({
                name: geo.properties.name,
                value: [geo.properties.center[0], geo.properties.center[1]-1, 'warning',"区域数据触发异常告警"],
                cityCode: parseInt(geo.properties.adcode)
              })
            }
            if(dangerIndex.includes(i)){
              dangerData.push({
                name: geo.properties.name,
                value: [geo.properties.center[0], geo.properties.center[1]-1, 'danger',"区域数据触发危急告警"],
                cityCode: parseInt(geo.properties.adcode)
              })
            }
          }
...
<style  lang="less">
.dashboard1080pA-map{.poa; .fullbox;
