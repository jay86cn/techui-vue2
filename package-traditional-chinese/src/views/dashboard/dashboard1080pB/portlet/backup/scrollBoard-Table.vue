<template>
    <dv-scroll-board v-if="!option.reset" :config="option" class="dashboardB-scrollBoard-table"  />
</template>
<script>
export default {
  name: "blockCenter",
  ...
  data() {
    return {
      simData:{
        name:['企業A企業A企業A企業A','企業B','企業C','企業D企業D企業D企業D','企業E','企業F'],
        number:[744,226,982,12,422,632],
        percent:[12,35,78,99,32,68],
      },
      option:{
        reset:true,
        header: ['企業名稱', '用水量(萬噸)', '超標比'],
        data:[],
        index: false,
...
<style lang="less">
.dashboardB-scrollBoard-table{.poa; left:10px; right:10px; top:40px; bottom:10px; width:auto;height:auto; 
  .header{
    .header-item{.ff("cn1"); }
  }
  .rows{
