  ...
  <div class="dashboard4X2Combin">
    ...
    <template v-else>
      <div class="mobileMode-alert">
        <p class="title">4x2組合屏 提示</p>
        <p>檢測到您當前處於移動端模式查看，4x2組合屏數據量龐大，通常移動設備的性能有限，無法支撐本功能的展示。 </p>
        <p>請移步到PC端訪問。 </p>
        <!-- <p>如果您已經在用pc端訪問，請確保您的瀏覽器窗口已經最大化，然後嘗試刷新。 </p> -->
      </div>
    </template>
  </div>
</template>
  ...
  data() {
    return {
      ...
      state:{
        ...
        init:0,
        dashboard:[
          {id:0,name: "數據屏A",component:"dashboardA" }, 
          {id:1,name: "數據屏B",component:"dashboardB" }, 
          {id:2,name: "數據屏C",component:"dashboardC" }, 
          {id:3,name: "數據屏D",component:"dashboardD" }, 
          {id:4,name: "數據屏E",component:"dashboardE" }
        ],
      }
    };
  },
    ...
    if(this.deviceType!="mobile"){
      ...
      if(tipState){
        $timer("notify1",()=>{
          $notify({
            title: '性能提示',
            message: '數據組合屏專為企業級的大屏展示而打造，需要更高性能的環境運行，因同時展現的圖表數據過多，面對一些弱性能的個人電腦環境較為吃力，如遇到卡頓，請更換電腦再試。也可嘗試在控制面板中關閉背景模糊、動畫效果等，提高頁面效率。控制面板開關位於窗口右上角',
            duration: 0
          });
        },3000)
        $timer("notify2",()=>{
          $notify({
            title: '提示',
            message: '可以通過拖拽右下角的面板，調換4x2拼接屏中的中間主屏內容。可以根據情況定制任意分辨率的大屏。 ',
            duration: 0
          });
        },5000)
