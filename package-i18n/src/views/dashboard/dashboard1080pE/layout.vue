  ...
  <div class="dashboard1080E">
   <div class="portlet-wrapper" v-for="(item,index) in portlets">
      <component :is='item.border' :config="getConfig(item)">
        ...
        <panelTitleA1 v-if="!item.hideTitle" :config="panelTitleConfig" >{{$t(`dashboardE.PTitle.${item.id}`)}}</panelTitleA1>
        <component :is='item.component'></component>
        <i>{{item.component}}</i>
      </component>
    </div>
    <systemTitleA3></systemTitleA3>
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    getConfig(item){
      let borderConfig={
        title:this.$t(`dashboardE.PTitle.${item.id}`),
        // titleColor:$c.cyl5,
