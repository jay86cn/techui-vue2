  ...
  methods: {
    ...
    processData(){
      ...
      xAxis.forEach((item,i)=>{
        this.chartData.xAxis[i]=this.$t(`dashboardC.l1.xAxis`)+(i+1)
      })
      legend.forEach((item,i)=>{
        this.chartData.legend[i]=this.$t(`dashboardC.l1.legend${i+1}`)
        processedData.push({
          name: this.$t(`dashboardC.l1.legend${i+1}`),//legend[i],
          type: type,
          //stack: '01',
          barWidth:15,
          label: {
