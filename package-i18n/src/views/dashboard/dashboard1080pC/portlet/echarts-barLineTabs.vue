...
<div class="chart-linebar-hasTabs-SA theme">
  <div class="card-header-tabs">
    <div class="button-wrap" @click="tabClick(index)" v-for="(item,index) in chartData" >
      <techButtonB1 :config="buttonConfig" :class="index==current?'active':''">{{$t(`dashboardC.c1.tab${index+1}`)}}</techButtonB1>
    </div>
  </div>
  ...
  methods: {
    ...
    processData(){
      ...
      xAxis.forEach((item,i)=>{
        this.chartData[this.current].xAxis[i]=this.$t(`dashboardC.c1.xAxis`)+(i+1)
      })
...
<style lang="less">
.chart-linebar-hasTabs-SA{height: 100%;
