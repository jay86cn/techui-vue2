<template>
  <div class="map-counter-wrap">
    <div class="counter-item" v-for="(item,index) in counterList">
      ...
      <div class="name">{{$t(`dashboardC.map.counter.item${index+1}`)}} <span v-if="false" class="unit">({{item.unit}})</span></div>
    </div>
  </div>
</template>
<script>
...
.dashboard1080pC-map{
  .map-counter-wrap{.poa; left:50%; .fixc("x"); top:100px; z-index: 10;
    .counter-item{.bd(var(--button-bd_hov)); .bgc(var(--popup-bg)); width: 190px; display:inline-block; .bgc(fade(@bll9,80%));  height: 60px; margin:0 20px 0 0; padding:10px 0 0 60px; position: relative;
