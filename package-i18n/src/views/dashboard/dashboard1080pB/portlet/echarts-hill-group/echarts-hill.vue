  ...
  methods: {
    ...
    processData(){
      ...
      xAxis.forEach((item,i)=>{
        this.chartData.xAxis[i]=this.$t(`dashboardB.r3.item${i+1}`)
      })
      data.forEach((item,i)=>{
        subData.push({
            value: data[i],
    ...
    processOption(){
      ...
      this.chartOption={
        update:false,
        title:{ text:this.$t(`dashboardB.r3.unit`), right:0, top:0, textStyle:{ color:getColor("staFCW"), fontSize:16, fontWeight:"normal" }, },
        tooltip: {
          trigger: 'axis',
          axisPointer: {
            type: 'none'
