...
<div class="chart-hasTabs-ScreenB">
  <div class="unit">{{$t(`dashboardB.r1.unit`)}}</div>
  <div class="card-header-tabs">
    <div class="button-wrap" @click="tabClick(index)" v-for="(item,index) in chartData" >
      <techButtonB1 :config="buttonConfig" :class="index==current?'active':''">{{$t(`dashboardB.r1.item${index+1}`)}}</techButtonB1>
    </div>
  </div>
  <div class="total-digital">
    <span class="desc">{{$t(`dashboardB.r1.item${current+1}`)}}</span>
  ...
  methods: {
    ...
    processData(){
      ...
      legend.forEach((item,i)=>{
        this.chartData[current].legend[i]=this.$t(`dashboardB.r1.item${current+1}_${i+1}`)
        processedData.push({
          value:data[i],
          name:legend[i],
          itemStyle:{
...
.chart-hasTabs-ScreenB{height: 100%;
  .card-header-tabs{position: absolute;  top:0; right:-10px; padding:0 10px; z-index: 10; white-space: nowrap; 
  ...
  .unit{.poa; left:10px;bottom:10px;.fc(var(--font-weak));}
}
