...
<div class="dashboardB-pieB-wrap echarts-hasSelect at1-left">
  <div class="total-digital">
    <span class="desc">{{$t(`dashboardD.l2.total`)}}</span>
    ...
    return {
      select:{
        options: [
          ...
          { value: 'b', label: this.$t("dashboardD.optionB.op2") }, 
        ],
        current: 'a'
      },
      chartData:{
  ...
  methods: {
    processData(){
      ...
      legend.forEach((item,i)=>{
        this.chartData.legend[i]=this.$t(`dashboardD.l2.legend${i+1}`)
        processedData.push({
          value:data[i],
          name:this.$t(`dashboardD.l2.legend${i+1}`),
          itemStyle:{
            color:colors[i]
          }
        })
