  ...
  methods: {
    processData(){
      ...
      legend.forEach((item,i)=>{
        this.chartData.legend[i]=this.$t(`dashboard4x2B.p1.legend${i+1}`)
        processedData.push({
          name: this.$t(`dashboard4x2B.p1.legend${i+1}`),
          type: "radar",
          symbol: "circle",
          color:colors[i],
          symbolSize: 10,
      ...
      indicator.forEach((item,i)=>{
        this.chartData.indicator[i].name=this.$t(`dashboard4x2B.p1.item${i+1}`)
      })
