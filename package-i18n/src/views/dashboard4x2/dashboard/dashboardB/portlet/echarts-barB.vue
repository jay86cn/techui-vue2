  ...
  methods: {
    processData(){
      ...
      xAxis.forEach((item,i)=>{
        this.chartData.xAxis[i]=this.$t(`dashboard4x2B.p4.xAxis`)+(i+1)
      })
      
      legend.forEach((item,i)=>{
        this.chartData.legend[i]=this.$t(`dashboard4x2B.p4.legend${i+1}`)
        processedData.push({
          name: this.$t(`dashboard4x2B.p4.legend${i+1}`),
          type: 'bar',
          //stack: '01',
          //barWidth:25,
          label: {
