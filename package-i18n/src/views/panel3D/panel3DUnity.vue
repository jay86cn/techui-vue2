...
<template>
  <adaptivePanel :config="adptivePanelConfig">
    ...
    <div class="title-desc">{{$t("panel3D.desc.unity")}}</div>
...
.gridDemoMax {.poa; .fullbox(40px); .grid(5,3,60px); //grid-auto-rows: minmax(100px, 200px);//声明行的高度
  .item{position: relative; padding:20px; 
    
  }
}
.adaptivePanel {
  .title-desc{.poa; top:20px; left:50%; .fixc("x"); font-size: 18px; .fc(@cyA10); .thidden;}
}
</style>